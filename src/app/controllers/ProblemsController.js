...
class ProblemsController {
    ...
    async runCode(req, res, next) {
        ...
        if (language === 'cpp') {
            ...
            const execCode = async (currentIndex, testcases, code, numParams, language) => {
                if (currentIndex < testcases.length) {
                    var newCode = await supportConvertCode(code, numParams, testcases[currentIndex], language);
                    console.log(newCode);
                    fs.writeFileSync(`test${currentIndex}.cpp`, newCode);

                    const scriptPath = `./test${currentIndex}.cpp`;
                    const buildPath = `output${currentIndex}`;
                    var result;
                    ...
                    currentIndex++;
                    execCode(currentIndex, testcases, code, numParams, language);
                } else {
            ...
            for (var i = 0; i < testcases.length; i++) {
                var newCode = await supportConvertCode(code, numParams, testcases[i], language);
                console.log(newCode);
            }
        }
    }
